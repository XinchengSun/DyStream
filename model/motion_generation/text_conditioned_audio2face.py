...
from transformers import CLIPTextModel, CLIPTokenizer, PreTrainedModel, Wav2Vec2Processor, Wav2Vec2Model, PretrainedConfig
...
class Audio2FaceGPT(nn.Module):
    def __init__(
        ...
        self.use_text_condition = bool(getattr(cfg, "use_text_condition", False))
        self.drop_text = float(getattr(cfg, "drop_text", 0.0))
        if self.use_text_condition:
            text_encoder_name = getattr(cfg, "text_encoder_name", "openai/clip-vit-base-patch32")
            text_dim = int(getattr(cfg, "text_dim", 512))
    ...
    def encode_text_condition(self, captions, batch_size, device):
        ...
        if len(captions) != batch_size:
            if len(captions) == 1:
                ...
                raise ValueError(f"Expected {batch_size} captions, got {len(captions)}")
        ...
        with torch.no_grad():
            text_hidden = self.text_encoder(**tokens).pooler_output
        return self.text_proj(text_hidden)

    def apply_text_condition(self, hidden_states, text_hidden, enable_dropout=False):
        if text_hidden is None:
            return hidden_states
        if enable_dropout and self.drop_text > 0:
            keep = (torch.rand(text_hidden.shape[0], 1, device=text_hidden.device) >= self.drop_text).float()
            text_hidden = text_hidden * keep
        return hidden_states + text_hidden.unsqueeze(1)
    ...
    def get_audio2face_fea(self, audio, prev_audio, n):
        ...
        audio_list = [i.detach().cpu().float().tolist() for i in audio]
        inputs = self.audio_processor(audio_list, sampling_rate=16000, padding=True)
        input_values = torch.tensor(inputs.input_values, dtype=torch.float32, device=audio.device)
        audio2face_fea = self.audio_encoder_face(input_values)["high_level"]
    ...
    def get_audio2face_fea_other(self, audio, prev_audio, n):
        ...
        inputs = self.audio_processor(audio_list, sampling_rate=16000, padding=True)
        input_values = torch.tensor(inputs.input_values, dtype=torch.float32, device=audio.device)
        audio2face_fea = self.audio_encoder_face_other(input_values)["high_level"]
        audio2face_fea = F.interpolate(
            audio2face_fea.transpose(1, 2), scale_factor=(self.cfg.pose_fps / 50), mode="linear", align_corners=True
        ).transpose(1, 2)
        if prev_audio is not None:
    ...
    def forward(self, face_latent_gt, noise_face_latent, time_step, audio, audio_other, prev_audio, prev_audio_other, anchor_latent, caption=None):
        ...
        text_hidden = self.encode_text_condition(caption, bs, device)
        audio_hidden = self.apply_text_condition(audio_hidden, text_hidden, enable_dropout=self.training)
    ...
    def one_clip_only_inference(
        ...
        text_hidden = self.encode_text_condition(caption, bs, device)
        audio_hidden_1 = self.apply_text_condition(audio_hidden_1, text_hidden)
        audio_hidden_2 = self.apply_text_condition(audio_hidden_2, text_hidden)
        audio_hidden_3 = self.apply_text_condition(audio_hidden_3, text_hidden)
        anchor_hidden = self.anchor_embed(anchor_latent)
    ...
    def inference(
        ...
        audio2face_fea = self.audio_encoder_face(
            torch.concat([input_values, torch.zeros([1, 80], device=input_values.device)], dim=-1)
        )["high_level"]
        audio2face_fea = F.interpolate(
            audio2face_fea.transpose(1, 2), scale_factor=(self.cfg.pose_fps / 50), mode="linear", align_corners=True
        ).transpose(1, 2)
        audio_other_list = [i.detach().cpu().float().tolist() for i in audio_other]
        inputs = self.audio_processor(audio_other_list, sampling_rate=16000, padding=True)
        input_values = torch.tensor(inputs.input_values, dtype=torch.float32, device=audio.device)
        ...
        feature_len = min(total_len, audio2face_fea.shape[1], audio_other2face_fea.shape[1])
        for i in range(0, feature_len, stride):
            start_idx = i
            end_idx = min(start_idx + window, feature_len)
